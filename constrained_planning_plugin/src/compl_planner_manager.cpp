...
{
class COMPLPlannermanager : public planning_interface::PlannerManager
{
public:
  COMPLPlannermanager() : planning_interface::PlannerManager()
  {
  }
  ...
  bool initialize(const robot_model::RobotModelConstPtr& model, const std::string& ns) override
  {
    for (const std::string& group : model->getJointModelGroupNames())
    {
      planning_contexts_[group] =
          COMPLPlanningContextPtr(new COMPLPlanningContext("chomp_planning_context", group, model));
    }
    return true;
  }
...
private:
  std::map<std::string, COMPLPlanningContextPtr> planning_contexts_;
};
}  // namespace compl_interface

CLASS_LOADER_REGISTER_CLASS(compl_interface::COMPLPlannermanager, planning_interface::PlannerManager);